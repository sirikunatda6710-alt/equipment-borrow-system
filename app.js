...
function escapeHtml(value) {
  ...
  async function setupDashboard() {
    ...
    let equipment = await loadEquipmentFromFirebase();

    // คำนวณสถิติจากระบบจำนวนชิ้น (total / available)
    let totalItems = 0;
    ...
    equipment.forEach(item => {
      ...
      const available = Number(item.available) >= 0 ? Number(item.available) : total;
      
      totalItems += total;

      if (item.status === 'unavailable') {
        unavailableItems += total;
      } else {
        availableItems += available;
        borrowedItems += (total - available);
      }
    });

    if (totalEquipmentEl) totalEquipmentEl.textContent = totalItems;
    if (availableEquipmentEl) availableEquipmentEl.textContent = availableItems;
    if (borrowedEquipmentEl) borrowedEquipmentEl.textContent = borrowedItems;
    if (unavailableEquipmentEl) unavailableEquipmentEl.textContent = unavailableItems;
    ...
    function renderEquipmentTable(searchText = '') {
      ...
      const filteredEquipment = equipment.filter(item => {
        ...
        return id.includes(keyword) || name.includes(keyword) || type.includes(keyword) || borrower.includes(keyword);
      });
      ...
      equipmentTable.innerHTML = filteredEquipment.map(item => {
        ...
        let state = 'พร้อมใช้งาน';
        let statusClass = 'status-available';

        if (item.status === 'unavailable') {
          state = 'ไม่พร้อมใช้งาน';
          statusClass = 'status-unavailable';
        } else if (item.available < item.total) {
          state = 'กำลังถูกยืม';
        ...
        return `
          <tr>
            ...
            <td>
              <button type="button" class="table-action-button" data-equipment-id="${escapeHtml(id)}">
                ดูรายละเอียด
              </button>
            </td>
          </tr>
      ...
      qsa('.table-action-button', equipmentTable).forEach(button => {
        button.addEventListener('click', () => {
          ...
          if (modal && modalContent) {
            let state = 'พร้อมใช้งาน';
            if (item.status === 'unavailable') state = 'ไม่พร้อมใช้งาน';
            else if (item.available < item.total) state = 'กำลังถูกยืม';

            modalContent.innerHTML = `
              <h2>${escapeHtml(item.name || 'รายละเอียดอุปกรณ์')}</h2>
              <p><strong>รหัส:</strong> ${escapeHtml(item.id || '-')}</p>
              <p><strong>ประเภท:</strong> ${escapeHtml(item.category || '-')}</p>
              <p><strong>จำนวนทั้งหมด:</strong> ${item.total || 1} ชิ้น</p>
              <p><strong>คงเหลือ (พร้อมใช้):</strong> ${item.available || 0} ชิ้น</p>
              <p><strong>สถานะ:</strong> ${escapeHtml(state)}</p>
              <p><strong>ผู้ยืม:</strong> ${escapeHtml(item.borrower || '-')}</p>
            `;
    ...
    if (headerSearch) {
      headerSearch.addEventListener('input', () => {
        ...
        if (tableSearch) tableSearch.value = value;
